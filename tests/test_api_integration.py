...
class TestMatchCreation:
    ...
    def test_create_match_with_all_fields(self, client):
        ...
        response = client.post(
            "/matches", json=payload, headers=headers("org1", "John", "Organizer")
        )
...
class TestMatchJoining:
    ...
    def test_join_match_success(self, client):
        ...
        # Join match
        response = client.put(
            f"/matches/{match['id']}/join", headers=headers("user1", "John", "Player")
        )
        assert response.status_code == 200
...
class TestErrorHandling:
    ...
    def test_nonexistent_match_operations(self, client):
        ...
        response = client.put(
            f"/matches/{non_existent_id}/join", headers=headers("user1")
        )
        assert response.status_code == 404

        # Leave non-existent match
        response = client.put(
            f"/matches/{non_existent_id}/leave", headers=headers("user1")
        )
        assert response.status_code == 404

        # Delete non-existent match
        response = client.delete(
            f"/matches/{non_existent_id}", headers=headers("user1")
        )
        assert response.status_code == 404
...
class TestComplexScenarios:
    ...
    def test_full_match_lifecycle(self, client):
        ...
        match = client.post(
            "/matches", json=payload, headers=headers("org1", "Organizer", "One")
        ).json()

        # Multiple users join
        client.put(
            f"/matches/{match['id']}/join", headers=headers("user1", "Player", "One")
        )
        client.put(
            f"/matches/{match['id']}/join", headers=headers("user2", "Player", "Two")
        )
